...
export default () => {
  ...
  const handleCodeChange = (e) => {
    ...
    else if (val.length === 4 && authCode.length === 3)
      setAuthCode(val.slice(0, -1) + '-' + val.slice(-1))
        ...
        <AnimatePresence>
          ...
          >
            ...
            >
              Authentication Code{' '}
              {
                /* error && error.message === "GraphQL error: Authorization code is invalid" */ errorInvalidCode && (
                  <S.Error>Invalid Code</S.Error>
                )
              }
            </S.Text>
            <S.Field>
              ...
              {showTooltip && (
                <S.Tooltip>
                  The auth code will have been sent to your email after you
                  purchased your MyFit bands.
                </S.Tooltip>
              )}
            </S.Field>
            <S.SignUp
              onClick={async (e) => {
                if (password && validate(email) && authCode.length === 7) {
                  ...
                  if (!loading) {
                    if (
                      authcodes.find((rec) => rec.authCode === authCode) !==
                      undefined
                    )
